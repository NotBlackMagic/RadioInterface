...
/**
  * @brief	This function gets the current SysTick Value in us + us in SysTick register
  * @param	None
  * @return	SysTick Value in us
  */
uint32_t GetSysTickus() {
	uint32_t us = (SysTick->VAL * 1000) / SysTick->LOAD;

	return (sysTickCnt * 1000 + us);
}
